...
def repexperiment_test(UgenFun, Model, hypothesis, statCal, randomBeta, Lapscale, Xdist, fixedU, testtype, sig, niter, n, K, ResponseGenVec, fitmodel, v1_1, v1_2, v1_3, v2_1, v2_2, v2_3, v3_1, v3_2, v3_3, p1_1, p1_2, p1_3, p2_1, p2_2, p2_3, p3_1, p3_2, p3_3):
    p2 = p2_1 + p2_2 + p2_3
    p3 = p3_1 + p3_2 + p3_3
    p23 = p2 + p3
    ...
    for j in range(niter):
        ...
        if fixedU == True:
            ...
            #========generate normal random values======
            # size of the U matrix
            sizeU = np.array([p23, K])
            #u_tmp = np.random.normal(loc = 0, scale = 1, size = (p23, K))
            u_tmp = UgenFun(sizeU)
            # standardize each column
            L2Norms = np.sum(np.abs(u_tmp)**2,axis=0)**(1./2)
